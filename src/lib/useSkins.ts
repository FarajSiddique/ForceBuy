...
export type LoadState =
	...
	| { status: typeof LoadStatus.Error; message: string }
	| { status: typeof LoadStatus.Ready; index: SkinIndex };
...
export function useSkins(): LoadState {
	const [state, setState] = useState<LoadState>({ status: LoadStatus.Loading });
	...
	useEffect(() => {
		...
		fetch(`${import.meta.env.BASE_URL}data/skins.json`)
			...
			.then((file) => {
				if (!cancelled)
					setState({ status: LoadStatus.Ready, index: buildIndex(file) });
			})
			.catch((err: unknown) => {
				if (!cancelled)
					setState({
						...
						message:
							err instanceof Error ? err.message : "Failed to load skins",
					});
			});
		return () => {
			cancelled = true;
